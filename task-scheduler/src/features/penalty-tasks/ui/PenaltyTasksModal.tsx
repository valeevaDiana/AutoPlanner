...
export const PenaltyTasksModal: React.FC<PenaltyTasksModalProps> = ({
  ...
  const [editingPenaltyTask, setEditingPenaltyTask] =
    useState<PenaltyTask | null>(null);
      ...
      return (
        ...
        >
          ...
          <div style={{ fontSize: "12px" }}>
            ...
            {getPositionText(task.timePositionRegardingTaskId)} задачи "
            {parentTaskName}"
          </div>
          {task.dateTimeRange && (
            <div style={{ fontSize: "12px" }}>
              <strong>Временной интервал:</strong>{" "}
              {getOperatorText(task.relationRangeId)} {task.dateTimeRange}
            </div>
          )}
          {task.startDateTimeRange && task.endDateTimeRange && (
            <div style={{ fontSize: "12px" }}>
              <strong>Доступный диапазон:</strong>{" "}
              {formatDate(task.startDateTimeRange)}{" "}
              {formatTime(task.startDateTimeRange)} -{" "}
              {formatDate(task.endDateTimeRange)}{" "}
              {formatTime(task.endDateTimeRange)}
            </div>
          )}
        </div>
      );
      ...
      return (
        ...
        >
          ...
          {task.startDateTimeRuleOneTask && (
            <div style={{ fontSize: "12px" }}>
              <strong>Возможно начать с:</strong>{" "}
              {formatDate(task.startDateTimeRuleOneTask)}{" "}
              {formatTime(task.startDateTimeRuleOneTask)}
            </div>
          )}
          {task.endDateTimeRuleOneTask && (
            <div style={{ fontSize: "12px" }}>
              <strong>Возможно закончить до:</strong>{" "}
              {formatDate(task.endDateTimeRuleOneTask)}{" "}
              {formatTime(task.endDateTimeRuleOneTask)}
            </div>
          )}
          {task.startDateTimeRange && task.endDateTimeRange && (
            <div style={{ fontSize: "12px" }}>
              <strong>Диапазон поиска времени:</strong>{" "}
              {formatDate(task.startDateTimeRange)}{" "}
              {formatTime(task.startDateTimeRange)} -{" "}
              {formatDate(task.endDateTimeRange)}{" "}
              {formatTime(task.endDateTimeRange)}
        ...
        >
          ...
          {task.startDateTimeRepit && (
            <div style={{ fontSize: "12px" }}>
              ...
              {formatDate(task.startDateTimeRepit)}{" "}
              {formatTime(task.startDateTimeRepit)}
            </div>
          )}
          {task.endDateTimeRepit && (
            <div style={{ fontSize: "12px" }}>
              <strong>Конец периода:</strong>{" "}
              {formatDate(task.endDateTimeRepit)}{" "}
              {formatTime(task.endDateTimeRepit)}
            </div>
          )}
        </div>
      );
  ...
  const convertPenaltyToTaskData = (
    ...
    return {
      ...
      startDateTimeRuleOneTask:
        penaltyTask.startDateTimeRuleOneTask || undefined,
  ...
  const getTaskForDeleteModal = (): Task | null => {
    ...
    return {
      ...
      realDate:
        taskToDelete.startDateTime?.split("T")[0] ||
  ...
  return (
    <>
      ...
      <TaskFormModal
        ...
        task={
          editingPenaltyTask
            ? (convertPenaltyToTaskData(editingPenaltyTask) as Task)
            : null
        }
        mode="edit"
      />
