...
const getMonthDates = (currentDate: Date) => {
  ...
  const daysCount = Math.ceil((lastDayOfMonth.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
...
const getTasksForDay = (date: Date, tasks: Task[], displayTasks: Task[]): TaskCell[] => {
  ...
  return (
    <div
      ...
      style={{
        ...
        cursor: 'pointer',
        position: 'relative',
        borderLeft: isStartDay ? `3px solid ${currentTheme.colors.textSecondary}` : 'none',
        borderRight: isEndDay ? `3px solid ${currentTheme.colors.textSecondary}` : 'none',
    ...
    >
      <div style={{ 
        ...
        {displayTime && <span style={{ fontWeight: 'bold', marginRight: '4px' }}>{displayTime}</span>}
        {task.title}
      </div>
    </div>
  );
...
export const MonthCalendar: React.FC<MonthCalendarProps> = ({
  ...
  const displayTasks = useMemo(() => splitAllTasks(tasks), [tasks, splitAllTasks]);
  const { weeks, month } = useMemo(() => getMonthDates(currentMonth), [currentMonth]);
  const goToPrevMonth = () => {
  ...
  return (
    <div className="calendar-container" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      {/* Навигация */}
      ...
      >
        ...
        <div style={{ minWidth: minTableWidth }}>
          ...
          <div className="month-calendar-header" style={{
            display: 'grid',
          ...
          {weeks.map((week, weekIndex) => (
            <div key={weekIndex} className="month-calendar-week" style={{
              display: 'grid',
              gridTemplateColumns: gridTemplateColumns,
              minHeight: '120px',
            }}>
                ...
                return (
                  ...
                  >
                    <div style={{
                      ...
                      color: isTodayDay ? currentTheme.colors.primary : currentTheme.colors.text,
                      marginBottom: '8px',
                      textAlign: 'right',
                    }}>
                      {date.getDate()}
